...
if (isset($_GET['user'])) {
    ...
    $db = new Database();
    $selectedUser    = mysqli_real_escape_string($db->conn, $selectedUser);
...
<body>
        ...
        <div class="account-info">
            ...
            <div class="user-list">
                ...
                <ul>
                    <?php
                $db = new Database();
                $qry = mysqli_query($db->conn, "SELECT Username FROM user_account WHERE UserType = 'ADMIN'");
                while ($row = $qry->fetch_assoc()) :
                $user = $row['Username'];
    
            ?>

                    <li><a href='index.php?view=adminchat&user=<?php echo $user ?>'><?php echo $user ?></a></li>
                    <?php endwhile; ?>
    ...
    $('#chat-form').submit(function(e) {
        ...
        aler('ddsad');
        $.ajax({
            url: 'NAVITEMS\submit_message.php',
            type: 'POST',
            data: {
                sender: sender,
                receiver: receiver,
